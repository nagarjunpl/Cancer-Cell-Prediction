...
if __name__ == "__main__":
    ...
    # Files that must exist for the API to work without re-training
    REQUIRED_MODEL_FILES = [
        "api_best_model.pkl",
        "api_preprocessor.pkl",
        "api_model_metadata.json",
        "api_feature_names.json",
    ]

    force_retrain = "--retrain" in sys.argv
    models_exist = all(os.path.exists(f) for f in REQUIRED_MODEL_FILES)

    if force_retrain or not models_exist:
        if force_retrain:
            print("\n" + "="*60)
            print("  --retrain flag detected: FORCING FULL PIPELINE RE-RUN")
            print("="*60)
        else:
            print("\n" + "="*60)
            print("  Model files not found — running full training pipeline...")
            print("="*60)
        ...
        print("\n" + "="*60)
        print("  FAST STARTUP — Trained model files detected, skipping pipeline.")
        print("  To force re-training, run:  python <script> --retrain")
        print("="*60)
    ...
    print(f"  http://localhost:{port}/redoc (ReDoc UI)")
    print("="*60)
